...
entity_type_mapping_file = os.path.join(
    cur_dir, 'resource', 'ace_to_aida_entity.tsv')
event_type_mapping_file = os.path.join(
    cur_dir, 'resource', 'ace_to_aida_event.tsv')
role_type_mapping_file = os.path.join(
    cur_dir, 'resource', 'ace_to_aida_role.tsv')
relation_type_mapping_file = os.path.join(
    cur_dir, 'resource', 'ace_to_aida_relation.tsv')
...
def get_span_mention_text(tokens, token_ids, start, end):
    ...
    for token, token_id in zip(tokens[start:end], token_ids[start:end]):
        ...
        try:
            assert len(text[:token_start] + token +
                    text[token_end + 1:]) == len(text)
            ...
            with open(f, 'r', encoding='utf-8') as r:
                for line in r:
                    ...
                    for i, (start, end, enttype, mentype, _) in enumerate(result['graph']['entities']):
                        ...
                        entity_id = 'Entity_EDL_{:07d}'.format(
                            len(entity_mapping) + 1)
                        entity_mapping[(sent_id, i)] = (
                            entity_text, entity_id, entity_span, enttype, mentype)
                        entity_id_mapping[entity_id] = (sent_id, i)
                        enttype_mapped = entity_type_mapping[enttype]
                        w.write(':{}\ttype\t{}\t1.000000\n'.format(
                            entity_id, enttype_mapped))
                    ...
                    for cur_trigger_idx, (start, end, eventtype, _) in enumerate(triggers):
                        ...
                        for trigger_idx, entity_idx, role, _ in roles:
                            if cur_trigger_idx == trigger_idx:
                                role_mapped = role_type_mapping['{}:{}'.format(
                                    eventtype, role).lower()]
                                _, entity_id, entity_span, _, _ = entity_mapping[(
                                    sent_id, entity_idx)]
