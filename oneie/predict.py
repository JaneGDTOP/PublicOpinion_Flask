...
def load_model(model_path, device=1, gpu=False, beam_size=5):
...
def predict(
    ...
    #     input_path, '*.{}'.format(file_extension)))
    ...
    for f in file_list:
        ...
        try:
            ...
            # write doc info
            if log_path:
                log_writer.write(json.dumps(doc_info, ensure_ascii=False) + "\n")
                log_writer.flush()
        except Exception as e:
            traceback.print_exc()
            if log_path:
                log_writer.write(
                    json.dumps({"file": f, "message": str(e)}, ensure_ascii=False)
                    + "\n"
                )
                log_writer.flush()
    progress.close()
...
parser.add_argument(
    "-m",
    "--model_path",
    default="./logs/cn/best.role.mdl",
    help="path to the trained model",
)
parser.add_argument(
    "-i",
    "--input_dir",
    default="./data/ace_cn/test.oneie.json",
    help="path to the input folder (ltf files)",
)
parser.add_argument(
    "-o",
    "--output_dir",
    default="./ace_outputs_cn",
    help="path to the output folder (json files)",
)
parser.add_argument("-l", "--log_path", default='./ace_outputs_cn/log', help="path to the log file")
parser.add_argument(
    "-c", "--cs_dir", default="./test_cs", help="path to the output folder (cs files)"
)
parser.add_argument("--gpu", default=True, help="use gpu")
parser.add_argument("-d", "--device", default=3, type=int, help="gpu device index")
...
parser.add_argument("--lang", default="chinese", help="Model language english/chinese")
parser.add_argument("--format", default="json", help="Input format (txt, ltf, json)")
