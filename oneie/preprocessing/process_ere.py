...
def sentence_tokenize(sentence: Tuple[int, int, str],
    ...
    for sent in sents:
        ...
        if index == -1:
            ...
            sents_.append((last + index + start, last +
                          index + len(sent) + start, sent))
        last += index + len(sent)
    return(sents_)
...
def read_annotation(path: str) -> (str, str, List[Entity], List[Relation], List[Event]):
    ...
    if entities_node:
        for entity_node in entities_node.find_all('entity'):
            ...
            for entity_mention_node in entity_node.find_all('entity_mention'):
                ...
                if mention_type == 'NOM':
                    mention_offset = int(
                        entity_mention_node.find('nom_head')['offset'])
                    mention_length = int(
                        entity_mention_node.find('nom_head')['length'])
                    ...
                    mention_text = entity_mention_node.find(
                        'mention_text').text
...
def extract(source_path: str,
    ...
    doc_id_, source_type, entities, relations, events = read_annotation(
        ere_path)
    ...
    sentence_relations = clean_relations(
