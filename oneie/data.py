...
from oneie.util import read_ltf, read_txt, read_json, read_json_single, read_customize_txt, read_input_text
...
def get_role_types(entities, events, id_map):
    ...
    for event_idx, event in enumerate(events):
        for arg in event['arguments']:
            entity_id = arg['entity_id']
            entity_id = id_map.get(entity_id, entity_id) #有重复就使用重复的id，一般情况下不会有重复的
...
class IEDataset(Dataset):
    ...
    def numberize(self, tokenizer, vocabs):
        ...
        for inst in self.data:
            ...
            entity_list = [
                (e['start'], e['end'], entity_type_stoi[e['entity_type']]) for e in entities]
            ...
            mention_list = [(i, j, l) for (i, j, k), l in zip(
                entity_list, mention_type_idxs)]
            ...
            trigger_list = [(e['trigger']['start'], e['trigger']['end'],
                             event_type_stoi[e['event_type']]) for e in events]
            ...
            role_type_idxs = [[role_type_stoi[l]
                               for l in ls] for ls in role_types]
            role_list = get_role_list(
                entities, events, entity_id_map, role_type_stoi)
...
class IEDatasetEval(object):
    ...
    def load_data(self):
        ...
        if self.input_format == 'txt':
            # doc_tokens, doc_id = read_customize_txt(self.path)
        ...
        elif self.input_format == 'input_text':
            doc_tokens,doc_id= read_input_text(self.path,self.language)
        else:
            raise ValueError(
                'Unknown input format: {}'.format(self.input_format))
        self.doc_id = doc_id
