...
def read_customize_txt(path):
    doc_id = os.path.basename(path)
    data = open(path, 'r', encoding='utf-8').read()
    sents = [s.strip() for s in data.split('\n') if s.strip()]
...
def read_input_text(input_text,language):
    if language=="chinese":
        tokenizer = BertTokenizer.from_pretrained('/media/ubuntu/projects/pretrainModel/bert-base-chinese', do_lower_case=False)
    else:
        tokenizer = BertTokenizer.from_pretrained("/media/ubuntu/projects/pretrainModel/bert-base-cased", do_lower_case=False)
    doc_tokens = []
    tokens = [(token, i, i + 1) for i, token in enumerate(tokenizer.tokenize(input_text))]
    doc_id="balabala"
    doc_tokens.append(("balabala",tokens))
    return doc_tokens, doc_id
